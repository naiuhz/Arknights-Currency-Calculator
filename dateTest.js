...
const timeDiffMinutes = Math.floor(
  (timeDiff / (1000 * 60 * 60) - timeDiffHours) * 60,
);
const timeDiffSeconds = Math.floor(
  ((timeDiff / (1000 * 60 * 60) - timeDiffHours) * 60 - timeDiffMinutes) * 60,
);
...
console.log(
  `Time until next day: ${Math.floor(
    timeDiffHours,
  )} hours, ${timeDiffMinutes} minutes and ${timeDiffSeconds} seconds.`,
);

let localResetMessage = `The day will reset on ${localNextDayReset.toLocaleDateString(
  'en-US',
  timeFormat,
)}`;
if (data.optional.timezoneCity) {
...
if (data.optional.sanityCap) {
  ...
  const sanityRechargeMinutes =
    data.optional.sanityCap - sanityRechargeHours * 10;
  const sanityRechargeTimeMilliseconds =
    sanityRechargeHours * 1000 * 60 * 60 +
    sanityRechargeMinutes * 1000 * 60 * 6;
  const startSanityRecharge = new Date(
    localNextDayReset.getTime() - sanityRechargeTimeMilliseconds,
  );
  if (localUTC < startSanityRecharge) {
    console.log(
      `If you wish to carry over today's sanity to tomorrow, have 0 sanity by local time: ${startSanityRecharge.toLocaleDateString(
        'en-US',
        timeFormat,
      )}`,
    );
  ...
  if (data.optional.currentSanity) {
    let excessSanity;
    if (localUTC < startSanityRecharge) {
      const excessSanityRechargeHours = Math.floor(
        (startSanityRecharge - localUTC) / (1000 * 60 * 60),
      );
      const excessSanityRechargeMinutes = Math.floor(
        (((startSanityRecharge - localUTC) / (1000 * 60 * 60) -
          excessSanityRechargeHours) *
          60) /
          6,
      );
      excessSanity =
        excessSanityRechargeHours * 10 + excessSanityRechargeMinutes;
      console.log(
        `You have excess sanity, use up all your current sanity (${
          data.optional.currentSanity
        }) and at least ${excessSanity} sanity that you'll save up up until local time: ${startSanityRecharge.toLocaleDateString(
          'en-US',
          timeFormat,
        )}.`,
      );
      ...
      const rechargedSanityHours = Math.floor(
        (localUTC - startSanityRecharge) / (1000 * 60 * 60),
      );
      const rechargedSanityMinutes = Math.floor(
        (((localUTC - startSanityRecharge) / (1000 * 60 * 60) -
          rechargedSanityHours) *
          60) /
          6,
      );
      const rechargedSanity =
        rechargedSanityHours * 10 + rechargedSanityMinutes;
      // console.log("rechargedSanity: " + rechargedSanity)
      if (data.optional.currentSanity > rechargedSanity) {
        console.log(
          `You have excess sanity, use up at least ${
            data.optional.currentSanity - rechargedSanity
          } sanity to prevent sanity overflow on ${localNextDayReset.toLocaleDateString(
            'en-US',
            timeFormat,
          )}`,
        );
      } else {
        const sanityOverflowLeft =
          data.optional.sanityCap - data.optional.currentSanity;
        const sanityOverflowLeftHours = Math.floor(sanityOverflowLeft / 10);
        const sanityOverflowLeftMinutes =
          sanityOverflowLeft - sanityOverflowLeftHours * 10;
        const sanityOverflowTime = new Date(
          localUTC.getTime() +
            sanityOverflowLeftHours * (1000 * 60 * 60) +
            sanityOverflowLeftMinutes * (1000 * 60),
        );
        console.log(
          `You do NOT have excess sanity to carry over to the next day. However you will overflow sanity on ${sanityOverflowTime.toLocaleDateString(
            'en-US',
            timeFormat,
          )}.`,
        );
